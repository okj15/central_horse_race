...
def get_sire_result(target_race):
    ...
    for target in target_race:
        record = list(
            Race.objects.filter(rank__isnull=False, horse__father=target.horse.father).values_list('rank', flat=True))
        sire_total_results[target.horse] = convert_record(record, target)

    # コース勝率
    sire_venue_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(venue__name=target.venue, rank__isnull=False,
                                          horse__father=target.horse.father).values_list('rank', flat=True))
        sire_venue_results[target.horse] = convert_record(record, target)

    # 枠勝率
    sire_bracket_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(bracket=target.bracket, rank__isnull=False,
                                          horse__father=target.horse.father).values_list('rank', flat=True))
    ...
    for target in target_race:
        record = list(Race.objects.filter(direction__name=target.direction, rank__isnull=False,
                                          horse__father=target.horse.father).values_list('rank', flat=True))
