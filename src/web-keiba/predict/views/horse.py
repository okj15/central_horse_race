...
def get_horse_result(target_race):
    ...
    for target in target_race:
        record = list(Race.objects.filter(venue__name=target.venue, rank__isnull=False, horse=target.horse).values_list('rank', flat=True))
        horse_venue_results[target.horse] =  convert_record(record)

    # 枠勝率
    horse_bracket_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(bracket=target.bracket, rank__isnull=False, horse=target.horse).values_list('rank', flat=True))
        horse_bracket_results[target.horse] =  convert_record(record)

    # コース * 枠勝率
    horse_venue_bracket_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(venue__name=target.venue, bracket=target.bracket, rank__isnull=False, horse=target.horse).values_list('rank', flat=True))
        horse_venue_bracket_results[target.horse] =  convert_record(record)

    # 馬場状態勝率
    horse_condition_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(track_condition__name=target.track_condition, rank__isnull=False, horse=target.horse).values_list('rank', flat=True))
        horse_condition_results[target.horse] =  convert_record(record)

    # 回り勝率
    horse_direction_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(direction__name=target.direction, rank__isnull=False, horse=target.horse).values_list('rank', flat=True))
