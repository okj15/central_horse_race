...
def get_jockey_result(target_race):
    ...
    for target in target_race:
        record = list(Race.objects.filter(venue__name=target.venue, rank__isnull=False, jockey=target.jockey).values_list('rank', flat=True))
        jockey_venue_results[target.horse] =  convert_record(record, target)

    # 枠勝率
    jockey_bracket_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(bracket=target.bracket, rank__isnull=False, jockey=target.jockey).values_list('rank', flat=True))
        jockey_bracket_results[target.horse] =  convert_record(record, target)

    # コース * 枠勝率
    jockey_venue_bracket_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(venue__name=target.venue, bracket=target.bracket, rank__isnull=False, jockey=target.jockey).values_list('rank', flat=True))
        jockey_venue_bracket_results[target.horse] =  convert_record(record, target)

    # 馬場状態勝率
    jockey_condition_results = dict()
    for target in target_race:
        record = list(Race.objects.filter(track_condition__name=target.track_condition, rank__isnull=False, jockey=target.jockey).values_list('rank', flat=True))
